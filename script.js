...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check API key
    if (!apiKey) {
        showToast('Silakan konfigurasi API Key Gemini untuk menggunakan fitur AI');
    }
});
...
async function callGeminiAPI(prompt) {
    ...
    if (!response.ok) {
        ...
        throw new Error(error.error?.message || 'Gagal memanggil API');
    }
...
Gunakan bahasa yang mudah dipahami siswa ${selectedKelas}.`;
        ...
        const materi = await callGeminiAPI(prompt);
